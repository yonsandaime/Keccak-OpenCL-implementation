...
__kernel void f_function(union INTER iState, local union INTER *State) {
  ...
  for (int round = 0; round < rounds; round++) {
    ...
    // Omega
    for (int i = 0; i < 5; i++) {
      CrossPlane[i] = state._64[i] ^ state._64[i + 5] ^ state._64[i + 10] ^
                      state._64[i + 15] ^ state._64[i + 20];
    }

    for (int i = 0; i < 5; i++) {
      D = CrossPlane[i == 0 ? 4 : (i - 1)] ^
          rotate(CrossPlane[i == 4 ? 0 : (i + 1)], (ulong)1);
      for (int y = 0; y < 5; y++) {
        state._64[i + y * 5] ^= D;
      }
    }
    ...
    for (int y = 0; y < 25; y += 5) {
      for (int x = 0; x < 5; x++) {
        state._64[x + y] = iState._64[x + y] ^ (~iState._64[(x + 1) % 5 + y] &
                                                iState._64[(x + 2) % 5 + y]);
      }
    }
