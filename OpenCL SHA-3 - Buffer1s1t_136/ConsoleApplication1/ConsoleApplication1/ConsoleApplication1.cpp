...
void hostr(const char* const filename)
{
    ...
    std::string kernel_code  = readFile("sha3Buffer1s1t_136.cl"); 
    ...
    while (size > 0) {
        ...
        hashed += pbuf->sgetn(contents, rateBytes);
        queue.enqueueWriteBuffer(buffer_Input, CL_TRUE, 0, rateBytes, contents);
    ...
    std::cout << "\n" << "Average Hashrate: " << (1 * sizeFichier) / avg_time;
    if (LOGG)myfile << filename << "; " << (1 * sizeFichier) / avg_time << "\n";
...
int main()
{
    if (LOGG)myfile.open("loggerBuffer1s1t136.csv");
    if (LOGG)myfile << "Buffer1s1t136" << "\n";
    hostr("other.bin");
