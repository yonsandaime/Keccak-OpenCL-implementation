...
void hostr(const char* const filename)
{
    for (int j = 0; j < 1; j++) {
        ...
        std::string kernel_code = readFile("sha3DualBuffer2s1t-5.cl");
        ...
        for (int offset = 0; size > 0; itter = !itter) {
            ...
            queue1.enqueueNDRangeKernel(kernelKeccak1, cl::NullRange, cl::NDRange(5), cl::NullRange, NULL, &prof_event);
            size -= rateBytesBuffer;
        ...
        queue.enqueueNDRangeKernel(kernelKeccak, cl::NullRange, cl::NDRange(5), cl::NullRange, NULL, &prof_event2);
...
int main()
{
    if (LOGG)myfile.open("loggerDualbuffer5Thread.csv");
    if (LOGG)myfile << "Dualbuffer5Thread" << "\n";
