...
void hostr(const char* const filename, int times = 1)
{
    ...
    std::string kernel_code  = readFile("sha3parallel5.cl"); 
    ...
    queue.enqueueNDRangeKernel(kernelKeccak, cl::NullRange, cl::NDRange(5), cl::NullRange, NULL, &prof_event);
...
int main()
{
    if (LOGG)myfile.open("loggerParallel5NoMalloc.csv");
    if (LOGG)myfile << "Parallel5NoMalloc" << "\n";
    hostr("other.bin");
