...
void hostr(const char* const filename, int times = 1)
{
    ...
    std::string kernel_code  = readFile("sha3parallel25.cl"); 
    ...
    queue.enqueueNDRangeKernel(kernelKeccak, cl::NullRange, cl::NDRange(25), cl::NullRange, NULL, &prof_event);
...
int main()
{
    if (LOGG)myfile.open("loggerParallel25NoMalloc.csv");
    if (LOGG)myfile << "Parallel25NoMalloc" << "\n";
    hostr("other.bin");
